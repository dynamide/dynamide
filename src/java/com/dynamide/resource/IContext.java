...
import java.util.Map;
 ...
 /* hidden notes:
   <pre>
    ...
    IContext
        permissions: a single permissions object. once the permissions are
                     locked, they can't be changed. But you can grant the
                     permission to change other things, like attributes and children.
               If attribute is com.dynamide.security.SecurityDecriptor,
                        then it describes whether it is mutable.
               -- OR: there is a standard attribute called permissions,
                      that is scrutinized when setting or changing.
        ...
        IContext children : (implements the Composite pattern)
        Object resourceObject : a single value object for leafs

    so context is just a Node
    and a subcontext is a child node
    but it has a non-rebindable attribute except by the class
    or permission that created it.
