...
public class Persistent extends JDOMFile implements IPropertyProvider {
    ...
    public void setProperty(String name, String value) throws DatatypeException {
        ...
        if (DEBUG_PROPS) System.out.println("setProperty(S,S)0: " + getID() + ':' + name + ':' + value + ';');
        Property property = getProperty(name);
        if ( property != null ) {
            if (DEBUG_PROPS) System.out.println("setProperty(S,S)1: "+property.dump()+';'+property.getClass().getName());
            property.setValue(value);
            if (DEBUG_PROPS) System.out.println("setProperty(S,S)2: "+property.dump()+';'+property.getClass().getName());
            if (DEBUG_PROPS) System.out.println("setProperty(S,S)3: "+value+';'+value.getClass().getName());
        } else {
            property = new Property(this, name, value);
            if (DEBUG_PROPS) System.out.println("setProperty(S,S)4: "+value+';'+value.getClass().getName());
        }
        setProperty(name, property);
    }
