 ...
 */
 public class WidgetType extends Persistent {
    ...
    public WidgetType(DynamideObject owner, String filename, Session session)
    throws JDOMException, IOException, DatatypeException {
    ...
    // %%%% this isn't in use yet, but it should be once you add caching of WidgetTypes.
    // you must be sure to make sure that the type is not bound to the session,
    // or figure out how to clone it efficiently.
    public void initializeForSession(Session session){
        setSession(session);
        if (!m_onImportFired) fire_onImport();
        //11/29/2003 2:10AM   if (!m_onLoadFired) fire_onLoad();
    }
    ...
    public String getDotName(){
        //This whole thing is weird, but you can't set anything in widget type, so being able to
        // uniquely identify one of these WidgetTypes in memory is not needed from Webmacro now.
        // So I just use the type discovered in the constructor to make a semi-unique name.
        //one of these will cause recursion when called here: getPropertyValue or getPropertyDefaultValue.
    ...
    //These get fired on WidgetType load, not per application, necessarily, since
    // we are allowed to cache WidgetTypes
    // %% This is fired from setElement(), so inline widgets will need to fire separately if they are ever implemented.
    //11/29/2003 2:09AM revamped it so that it is per-instance-on-a-page.
    ...
    public ScriptEvent fire_onLoad(Widget widget){
        ...
        //logDebug("############ fire_onLoad: "+eventName + " filename: "+getFilename() + " RC: "+ ev.printResultCode());
        m_onLoadFired = true;
        return ev;
    }
    ...
    public ScriptEvent fire_onImport(){
        String widgetTypeName = AbstractWidget.widgetTypeToScriptName(getType());
        //logDebug("widgetTypeName: "+widgetTypeName);
        //logDebug("getType: "+getType());
        ...
        //logDebug("############ fire_onImport: "+eventName + " filename: "+getFilename()+" RC: "+ ev.printResultCode());
