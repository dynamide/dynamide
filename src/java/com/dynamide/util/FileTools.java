...
public class FileTools implements IFileDiverListener {
    ...
    private static final boolean debug = false; //controls if System.out is used for dumping debug info.
    ...
    public static class FileInfo {
        ...
        public boolean getNotModified(){return m_notModified;}
        public void setNotModified(boolean new_value){m_notModified = new_value;}

        private boolean m_fileNotFound = false;
        public boolean getFileNotFound(){return m_fileNotFound;}
        public void setFileNotFound(boolean new_value){m_fileNotFound = new_value;}

        private boolean m_isBinary = false;
        public boolean getIsBinary(){return m_isBinary;}
        protected void setIsBinary(boolean new_value){m_isBinary = new_value;}

        private File m_file = null;
        public File getFile(){return m_file;}
        protected void setFile(File new_value){m_file = new_value;}

        private String  m_asString = "";
        public String  getAsString(){return m_asString;}
        protected void setAsString(String  new_value){m_asString = new_value;}

        private byte[] m_bytes = new byte[0];
        public byte[] getBytes(){return m_bytes;}
        protected void setBytes(byte[] new_value){m_bytes = new_value;}

        private boolean m_empty = false;
        public boolean isEmpty(){return m_empty;}
        protected void setEmpty(boolean new_value){m_empty = new_value;}
    }

    //=========== IFileDiverListener interface ====================================
    ...
    public static void directoryDiver(String diveID, File directory, String relativePath, IFileDiverListener listener, String separator){
        ...
        if (files == null || files.length == 0){
            if (debug) Log.error(FileTools.class, "null or zero length name");//empty directory, I think.
            return ;
        }
        String newname;
        for ( int i = 0; i<files.length; i++ ) {
            ...
            if ( file != null && ! file.isDirectory() ) {
                newname = relativePath.length() > 0 ? relativePath+separator+file.getName ()
                                                    : file.getName();
    ...
    public static Map list(String dirname, int type, int nameStyle)
    throws Exception {
        Hashtable hashtable = new Hashtable();
        File directory = new File(dirname);
        if ( directory.exists() ) {
            String[] files = directory.list();
            if (files != null && files.length > 0){  //protect against empty dir.
                for ( int i = 0; i<files.length; i++ ) {
                    boolean doit = false;
                    if ( files[i].equals(".") || files[i].equals("..") ) {
                        continue;
        ...
        try {
            if (f.exists()){
                ...
                if (f.exists()) {
                    ...
                    Log.error(FileTools.class, "FileTools.createDirectory could not mkdirs for \""+baseDir+"\",\""
                                                 +relativePath+"\" path: \""+f.getCanonicalPath()+"\" exists: "+f.exists());
                }
            }
        } catch (Exception e){
    ...
    public static void cleanOneDir(String dir, String name, String sanity){
        //System.out.println("cleanOneDir: "+dir+name+sanity);
        ...
        for ( int i = 0; i<files.length; i++ ) {
            if (files[i].startsWith(sanity)){  //"" always matches.
                File file = (new File(dirname,  files[i]));
                if (file!=null){
                    //System.out.println("cleanOneDir DELETING: "+dirname+sanity+" --> "+file.toString());
                    file.delete();
                }
            }
        }
     ...
     */
    public static List copyDir(String srcDir, String destDir)
    throws IOException {
    ...
    private static List copyDir(File directory, String relativePath, String separator, List outputList)
    throws IOException {
        Log.debug(FileTools.class, "in copyDir: "+directory+" path: "+relativePath+" sep: "+separator);
        String[] files = directory.list();
        if (files == null || files.length == 0){
            Log.error(FileTools.class, "null or zero length name");//empty directory, I think.
            return outputList;
        }
    ...
    public static File openFile(String dir, String relPath, boolean forceParentDirectories){
        ...
        if ( theFile != null ) {
            try{
                //System.out.println("FileTools.openFile opened: "+theFile.getCanonicalPath());
            } catch (Exception e){
                System.out.println("FileTools.openFile opened: "+relPath + " in "+dir +" " + theFile.toString());
            }
        } else {
            //Log.debug(FileTools.class, "
            //System.out.println("FileTools.openFile could not open: "+relPath + " in "+dir);
        }
        return theFile;
    }
    ...
    throws IOException {
        //River of shit.  This was supposed to work.
        //StringReader s = new StringReader(path);
        //StreamTokenizer st = new StreamTokenizer(s);
        //st.nextToken();
        //return st.sval;
        //Instead, let's do brain-dead trimming of shell quote characters.
        ...
        //Log.debug(FileTools.class, "******************88 resolve(\""+path+"\") == "+result);
        //System.out.println("******************88 resolve(\""+path+"\") == "+result);
        return result;
    }
            ...
            if (!theFile.isDirectory()){
                if (lastModified > 0){
                    if ( (theFile.lastModified() - lastModified) < 0){
                        //Log.debug(FileTools.class, "NOT modified. lastModified: "+lastModified+" theFile: "+theFile.lastModified());
                        FileInfo result = new FileInfo(false, false, theFile);
                        result.setNotModified(true);
                        return result;
                    }
                }
                //Log.debug(FileTools.class, "WAS modified lastModified: "+lastModified+" theFile: "+theFile.lastModified());
    ...
    public static FileInfo readBinaryFileInfo(String dir, String relPath, long lastModified){
     ...
     //2013-03-17 added from stackoverflow discussion, method to pull InputStream into a String: 
     ...
     */
    public static String copyToTempFile(String directory, String shortFilename, String extension)
    throws Exception {
        int i = 0;
    ...
    /** Copies the file named by fileName to a backup file in the same directory,
     *   with a name generated by  FileTools.generateBackupName(fileName) and returns true, otherwise
     *   returns true, and returns false if there's an Exception.
     */
    public static boolean backup(String fileName){
        try {
            if ((new File(fileName)).exists()){
                String tempName = FileTools.generateBackupName(fileName);
                FileTools.copyFile(fileName, tempName);
            }
            return true;
        } catch (Exception e){
            Log.error(FileTools.class, "ERROR in backup of file: "+fileName, e);
            return false;
        }
    }
    ...
    public static Properties loadPropertiesFromFile(String dir, String relativeName){
        try {
            ...
            //logError("ERROR: in loadPropertiesFromFile", e); //don't need this to std-out, result of null implies file not found, etc.
        }
        return null;
    }
    ...
    public static void main(String args[]){
        ...
        if ( opts.getOptionBool("-testDiver") ) {
            ...
            directoryDiver("FileTools.main", "C:\\temp", tools);  //local test in main().
        }
