//order is important:
// call setjava14.bat
// set classpath=c:\bin\jaxen-1.0-RC-1\jaxen-full.jar;%classpath%
 ...
 *  Here are some examples of what you can do:
  * <pre>
        //You instantiate this class like so:
        XDB xdb = new XDB();
        xdb.openXML("myXml", myXmlFilename);
        xdb.openXML("myOtherXml", myOtherXmlFilename);

        //Here are some examples of what you can do:
        //These are not in any order or necessarily compilable, but each statement is valid:

        XPath xp = xdb.prepare("vars/var[@name='datetime']/value");
        out.println("<valueOf name='datetime'>"+xp.valueOf(rule)+"</valueOf>");
        out.println("<valueOf name='frequency'>"+xdb.valueOf(rule, "vars/var[@name='frequency']/value")+"</valueOf>");
        Element ruleDefinition = xdb.selectFirst("allrules", "/rules/ruleDefinition[@id='"+type+"']");
        Element ruleDefVars = xdb.selectFirst(ruleDefinition, "vars");
        Element ruleVars = xdb.selectFirst(rule, "vars");
        #foreach (Element var in xdb.select(ruleVars, "var"))
            <aRuleVar>${xdb.outputElement(var)}</aRuleVar>
        #end
 * </pre>
 *
 */
public class XDB extends DynamideObject {
    //NOTE: METHODS ARE STORED ALPHABETICALLY, AFTER GETTERS/SETTERS.

    //================== Constructors ==============================================
    public XDB(){
        this(DEFAULT_SAX_DRIVER_CLASS);
        //System.out.println("Creating XDB: "+toString());
    }
    ...
    //================== variables, and simple getters/setters =====================
    ...
    //A list of all root elements opened, stored by a user-supplied name.
    //The class stored is always an Element, the key is a String.
    ...
    public String getDefaultStoreName(){return m_defaultStoreName;}
    ...
    public void setDefaultStoreName(String new_value){m_defaultStoreName = new_value;}
    ...
    /*
    public String dumpRulesXml(String rulesFilename, String query){
        String result;
        try {
            String rulesXml = readFile(rulesFilename);
            Document rulesDocument = DocumentHelper.parseText(rulesXml);
            result = outputNode(rulesDocument.selectSingleNode(query));
        } catch (Exception e){
            result = "ERROR: [dumpRulesXml] "+e;
            addError(result);
        }
        return result;
    }
    */
    ...
    public static String outputElement_INNER(Element el, boolean contentOnly){
        try {
            ...
            XMLOutputter xmloutputter = com.dynamide.JDOMFile.createJDomXMLOutputter(false/*expandEmptyElements*/);
            ...
            } catch (Exception e)  {System.out.println("ERROR: couldn't reset() bos in XDB "+e);}
            return result;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /*
    public boolean mergeIntoElement(Branch source, Branch target){
        Iterator it = source.nodeIterator();
        while ( it.hasNext() ) {
            Node child = (Node)it.next();
            Node childClone = (Node)child.clone();
            if (childClone instanceof Branch){
                ((Branch)childClone).detach();
                target.add(childClone);
            } else if (childClone instanceof Text) {
                target.add(childClone);
            } else {
                System.out.println("NOT ADDING: "+childClone.getText());
            }
        }
        return true;
    }
   ...
   /*public void saveDocument(Document document){
        try {
            String docString = outputNode(document.getRootElement());
            // todo, save file.  %% System.out.println("document: "+sw.toString());
            System.out.println("WARNING: saveDocument not implemented.");
        } catch (Exception e){
            System.out.println("ERROR: [saveDocument] "+e);
        }
    }*/
    ...
    public synchronized boolean saveXML(String xmlRepositoryName, String directory, String filename){
        String content = "<?xml version='1.0' encoding='UTF-8'?>"
                         ...
                         +getXMLString(xmlRepositoryName);
    ...
    throws Exception {
        ...
        //new org.jaxen.JaxenException("test");
        XDB xdb = new XDB();
        ...
        //System.out.println(xdb.valueOf(storeName, xpath));
        //System.out.println(xdb.outputElement(xdb.selectFirst(storeName, xpath)));
